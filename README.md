...
The answer was never a smarter model. It's a foundation: **map the codebase, guard the dangerous edits, and keep the AI coherent with how you build.**
...
- **Readiness** — *can AI work here safely?* A 0–100 score, weighted by **risk, not size**, across nine signals, with a ranked punch list of what to fix next.
- **Coherence** — *does what the AI wrote belong?* ReadyBase learns your conventions from the code and git history and flags where new work has drifted from them. Readiness gets AI in the door; coherence keeps it on your rails.
...
readybase update --check  # just tell me if there's a newer version
```
...
```bash
readybase go .            # one command: scan, write guidance, index, and set up
readybase score .         # AI-readiness score (0–100) with the fix list
...
### 🗺 Map
A structural graph of your repo: direct and transitive dependents (**blast radius**), co-change partners mined from git history, ownership and bus factor, complexity hotspots, and a per-file intelligence card. Importance is ranked by **graph centrality** — a file every dependency path routes through is load-bearing even if few files import it directly, which raw dependent-counts miss.

```bash
readybase relate internal/storage/db.go   # blast radius + deps + tests, in one call
...
### 🛡 Guard
ReadyBase writes the guidance your assistant reads (`CLAUDE.md`, `AGENTS.md`, guard rules) and installs optional git hooks. When your AI is about to edit a high-blast-radius file, the warning arrives at the right moment — no prompt engineering, fail-open by default, the guidance travels with the repo.

### 📊 Govern
One readiness score with a ranked punch list ordered by **points-per-hour**, so the cheapest wins come first. Re-scan any time; `readybase diff .` proves the number moved. A manager-facing readiness summary and portfolio ranking (`readybase portfolio <dir>`) roll many repos up into one view.

### 🎚 Watch (coherence)
ReadyBase learns your naming, structure, and error-handling conventions by consensus — not a style guide nobody reads — and flags where AI-written code has strayed. Drift caught in the moment keeps the codebase feeling like one author.

---

## The AI-readiness score

One number from 0 to 100, composed from nine signals, **weighted by risk rather than size** so a large well-owned repo isn't punished and a fragile one can't hide. No fake headline: it's computed from your repo on disk and every signal is inspectable.
...
|---------|----------------|
| **CLI-backed skills** (default) | `readybase go .` installs lightweight skills so `explore` / `relate` / `story` are available with **no always-on MCP server** — zero idle token cost |
| **MCP** (opt-in) | `readybase mcp` registers the tools: `readybase_explore` (start here), `readybase_relate` (blast radius + deps + tests), `readybase_story` (history + authors), plus the full set — `score`, `coverage`, `convention`, `who_owns`, `ghost`, `succession`, and more |
| **Git hooks** | `readybase hooks install` — blast-radius warning on commit, readiness gate on push, session context on start |
...
The action emits `score`, `grade`, `ai-readiness`, and `passed`, and writes a job summary. Add `sarif: true` (with `permissions: security-events: write`) to surface findings in the **Security** tab and as inline PR annotations. `readybase export` also emits scorecards for Backstage / Port.io / Cortex.
...
- **It judges, not just retrieves.** A readiness score, a coherence verdict, and a guard that gates the risky edit — not only a search index. It tells the AI *whether a change belongs*, not just *what's in the file*.
...
- **Cost-first.** CLI-backed skills by default, so there's no always-on MCP server burning tokens on every session. Context is budgeted to the significant minority, not dumped wholesale.
...
| Community detection (Blondel et al., *Louvain*) & centrality (Brandes, *betweenness*) | Finding architectural modules and load-bearing files in a graph | research |
